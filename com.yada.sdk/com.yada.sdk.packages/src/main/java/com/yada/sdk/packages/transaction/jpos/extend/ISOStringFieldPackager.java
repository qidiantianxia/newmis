package com.yada.sdk.packages.transaction.jpos.extend;
...
public class ISOStringFieldPackager extends ISOFieldPackager {
	...
	public ISOStringFieldPackager(EncodingLiteralInterpreter interpreter,
			Prefixer prefixer) {
		super();
	...
	public ISOStringFieldPackager(int maxLength, String description,
			EncodingLiteralInterpreter interpreter, Prefixer prefixer) {
	...
	public byte[] pack(ISOComponent c) throws ISOException {
		try {
			...
			if (rawData.length > getLength()) {
				throw new ISOException("Field length " + rawData.length
			...
			if (prefixer == NullPrefixer.INSTANCE) {
				...
				byte[] retData = new byte[prefixer.getPackedLength()
						+ rawData.length];
